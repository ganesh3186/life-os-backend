...
const getInsights = async (userId, type = "monthly") => {
    const goals = await goal_model_1.default.find({ userId });
    const now = new Date();
    /* ---------------- DATE RANGE ---------------- */
    ...
    const uniqueDates = Array.from(new Set(allDates.map((d) => new Date(d).toDateString()))).map((d) => new Date(d));
